...
// define self.__sw__.init()
{
  sw.init = ({
    ...
    if (typeof resources !== "object") {
      throw new TypeError("resources should be an object, got ".concat(resources));
    }
    if (typeof name !== "string") {
      throw new TypeError("name should be a string, got ".concat(name));
    }
    if (name.length === 0) {
    ...
    if (typeof logBackgroundColor !== "string") {
      throw new TypeError("logBackgroundColor should be a string, got ".concat(logBackgroundColor));
    }
    if (typeof logColor !== "string") {
      throw new TypeError("logColor should be a string, got ".concat(logColor));
    }
        ...
        await Promise.all(urlsToCache.map(async url => {
          ...
          try {
            ...
            if (response.status === 200) {
              logger.info("put \"".concat(asRelativeUrl(request.url), "\" into cache"));
              installed += 1;
            } else {
              logger.warn("cannot put ".concat(request.url, " into cache due to response status (").concat(response.status, ")"));
            }
          } catch (e) {
            logger.warn("cannot put ".concat(request.url, " in cache due to error while fetching: ").concat(e.stack));
          }
        }));
        if (installed === total) {
          logger.info("install done (".concat(total, " resources added in cache)"));
        } else {
          logger.info("install done (".concat(installed, "/").concat(total, " resources added in cache)"));
        }
      };
    }
    ...
    // --- activation phase ---
    {
      ...
      const handleActivateEvent = async () => {
        ...
        await Promise.all(cacheKeys.map(async cacheKey => {
          if (cacheKey !== cacheName && cacheKey.startsWith("".concat(name, "_"))) {
            logger.info("delete old cache \"".concat(cacheKey, "\""));
      ...
      const handleFetchEvent = async fetchEvent => {
        ...
        if (request.mode === "navigate") {
          ...
          if (preloadResponsePromise) {
            ...
            if (preloadResponse) {
              logger.info("".concat(relativeUrl, " -> use preloaded response"));
        ...
        try {
          const request = fetchEvent.request;
          logger.debug("open ".concat(cacheName, " cache"));
          const cache = await self.caches.open(cacheName);
          logger.debug("search response matching this request in cache");
          const responseFromCache = await cache.match(request);
          if (responseFromCache) {
            logger.info("".concat(relativeUrl, " -> use cache"));
            return responseFromCache;
          }
          logger.info("".concat(relativeUrl, " -> delegate to navigator"));
          return self.fetch(request);
        } catch (e) {
          logger.warn("error while trying to use cache for ".concat(relativeUrl, " -> delegate to navigator"), e.stack);
          return self.fetch(request);
        }
      };
...
const createCacheName = (() => {
  ...
  return cachePrefix => {
    ...
    return "".concat(cachePrefix, "_").concat(timestamp).concat(random);
  };
})();
const createLogger = ({
  logLevel,
  ...
  const injectLogStyles = args => {
    return ["%cjsenv%csw", "background: orange; color: rgb(55, 7, 7); padding: 1px 3px; margin: 0 1px", "background: ".concat(logBackgroundColor, "; color: ").concat(logColor, "; padding: 1px 3px; margin: 0 1px"), ...args];
  };
  const logger = {
    debug: (...args) => {
      if (logLevel === "debug") {
